...
public class Boggle {
	...
	public static String searchWords(TrieNode root, char boggle[][], int index, int jIndex, String str) {
		if (root.endOfWord) {
			return str;
		}
		...
		for (int index = 0; index < boggle.length; index++) {
			for (int jindex = 0; jindex < boggle[index].length; jindex++) {
				if (root.children[boggle[index][jindex] - 'A'] != null) {
					System.out.println(searchWords(root, boggle, index, jindex, boggle[index][jindex] + ""));
				}
			}
		}
	}
